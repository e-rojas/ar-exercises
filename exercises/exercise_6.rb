...
4. Go ahead and create some more employees using 
the create method. You can do this by making multiple calls to create 
    (like you have before.) No need to assign the employees to variables though.
     Create them through the `@store#` instance variables that you defined in previous exercises.
      Create a bunch under `@store1` (Burnaby) and `@store2` (Richmond). Eg: `@store1.employees.create(...)`.=begin 

=end
@store1.employees.create(first_name: "Khurram", last_name: "Virani", hourly_rate: 60);
@store1.employees.create(first_name:"Sam",last_name:"Thompson",hourly_rate:89)
@store2.employees.create(first_name:"Sandy",last_name:"Williams",hourly_rate:44);
@store2.employees.create(first_name:'Ryan',last_name:'Goslin', hourly_rate:33);